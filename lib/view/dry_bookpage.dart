...
class DryBookPage extends StatefulWidget {
  final Dry dry;
  final User user;
  final Slot slot;
  const DryBookPage({Key? key, required this.dry, required this.user, required this.slot})
      : super(key: key);

  @override
  State<DryBookPage> createState() => _DryBookPageState();
}

class _DryBookPageState extends State<DryBookPage> {
  ...
  void initState() {
    ...
    try {
      final response = await http.get(
        Uri.parse('${ServConfig.Serv}php/get_booked_dry.php'),
      );
      if (response.statusCode == 200) {
        final data = json.decode(response.body) as List;
        bookedSlots = data
            .map<Map<String, dynamic>>(
                (dry) => Map<String, dynamic>.from(dry))
            .toList();

        bookedSlotsByCurrentUser = bookedSlots
            .where((dry) => dry['user_id'] == widget.user.id)
            .toList();

        setState(() => isLoading = false);
      } else {
        showErrorDialog(
          "Error fetching slots: Server responded with status code ${response.statusCode}",
        );
      }
    } catch (e) {
  ...
  bool isSlotBooked(
    ...
    for (var dry in bookedSlots) {
      if (dry['dry_date'] == selectedDateString &&
          dry['dry_time'].startsWith(selectedStartTime) &&
          dry['dry_time'].endsWith(selectedEndTime) &&
          dry['user_id'] == userId) {
        return true;
      }
    }
  ...
  final selectedSlot = getFormattedTime();
  final isSlotAlreadyBooked = isSlotBooked(today, selectedSlot, widget.user.id);

  if (!isSlotAlreadyBooked) {
    final url = Uri.parse('${ServConfig.Serv}php/dry_book_appointment.php');
    final response = await http.post(
      ...
      body: {
        ...
        'appointment_type': 'dry', // Pass 'dry' directly
      },
    );

    if (response.statusCode == 200) {
      // Assuming you have a user object available for passing to SuccessPage
      print(response.body);
      Navigator.pushReplacement(
        // Using pushReplacement to prevent going back to the booking page
        context,
        MaterialPageRoute(
          builder: (context) =>
              SuccessPage(user: widget.user, dry: widget.dry, slot: widget.slot,),
        ),
      );

      // After booking, update the list of booked slots and recheck slot availability
      fetchBookedSlots();
    } else {
  ...
  int getSlotsForDay(DateTime day) {
    ...
    int bookedCount =
        bookedSlots.where((dry) => dry['dry_date'] == formattedDate).length;
          ...
          Container(
            ...
            child: GridView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final isSlotAlreadyBooked =
                    isSlotBooked(today, dry, widget.user.id);
                final bool isSelected = selectedTimeIndex == index;

                // Calculate the number of users booked at the same time
                int usersBookedAtSameTime = getUsersBookedAtSameTime(dry);

                return isSlotAlreadyBooked
                    ? _buildBookedSlot(dry)
                    : _buildAvailableSlot(
                        dry, isSelected, index, usersBookedAtSameTime);
              },
            ),
          ),
        ],
  ...
  Widget _buildAvailableSlot(
      String dry, bool isSelected, int index, int usersBookedAtSameTime) {
    final isSlotAlreadyBooked = isSlotBooked(today, dry, widget.user.id);
    ...
    final List<String> times = dry.split('-');
  ...
  Widget _buildBookedSlot(String dry) {
    // Split the slot text into start and end times
    final List<String> times = dry.split('-');
    final String startTime = times[0].trim();
    final String endTime = times[1].trim();
  ...
  int getUsersBookedAtSameTime(String dry) {
    ...
    return bookedSlots
        .where((dryData) =>
            dryData['dry_date'] == formattedDate &&
            dryData['dry_time'].startsWith(startTime) &&
            dryData['dry_time'].endsWith(endTime))
        .length;
  }
